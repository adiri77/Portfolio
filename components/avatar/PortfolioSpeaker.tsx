...
export default function PortfolioSpeaker() {
  ...
  return (
    <motion.div
      className="speaker-panel relative mx-auto w-full overflow-hidden rounded-3xl border border-indigo/25 bg-gradient-to-br from-cream via-cream-warm to-indigo-pale/40 shadow-[0_16px_48px_rgba(79,70,229,0.14)]"
      initial={{ opacity: 0, y: 24 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.7, ease: "easeOut" }}
    >
      <div className="pointer-events-none absolute -right-16 -top-16 h-48 w-48 rounded-full bg-indigo/10 blur-3xl" />
      <div className="pointer-events-none absolute -bottom-20 -left-12 h-40 w-40 rounded-full bg-indigo-light/20 blur-3xl" />
        ...
        <div className="flex w-full max-w-[11rem] shrink-0 flex-col items-center gap-3">
          <motion.div
            className={`speaker-portrait relative mx-auto aspect-[4/5] w-full overflow-hidden rounded-2xl border-4 bg-cream shadow-lg ${
              isSpeaking ? "border-indigo shadow-[0_0_28px_rgba(99,102,241,0.35)]" : "border-indigo/30"
            }`}
          ...
          >
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-indigo-dark/25 via-transparent to-transparent" />

            <AnimatePresence>
              {isSpeaking && (
                <motion.div
                  className="absolute inset-x-0 bottom-0 flex items-end justify-center gap-1 bg-gradient-to-t from-indigo-dark/50 to-transparent px-3 pb-3 pt-8"
          ...
          <div>
            <p className="text-sm font-bold text-indigo-dark">Aditya Singh</p>
        ...
        <div className="flex w-full min-w-0 max-w-xl flex-col items-center gap-4 lg:max-w-lg">
          <div className="flex w-full flex-col items-center gap-3">
            <div className="min-w-0">
              <h2 className="text-lg font-bold text-indigo-dark sm:text-xl">Hear my story</h2>
              <p className="mt-0.5 text-sm text-text-secondary">
                Tap a topic or press play — I&apos;ll narrate my portfolio for you.
              </p>
            </div>

            <div className="flex shrink-0 items-center justify-center gap-2 rounded-full border border-indigo/20 bg-cream/80 px-3 py-1.5">
              <motion.span
                className={`inline-flex h-2 w-2 shrink-0 rounded-full ${isSpeaking ? "bg-indigo" : "bg-indigo/40"}`}
                animate={isSpeaking ? { scale: [1, 1.4, 1] } : {}}
                transition={{ duration: 0.9, repeat: isSpeaking ? Infinity : 0 }}
              />
              <span className="text-xs font-semibold text-indigo-dark">
          ...
          <div className="speaker-transcript relative w-full min-h-[8rem] rounded-2xl border border-indigo/20 bg-cream/90 text-left shadow-inner sm:min-h-[9rem]">
            <div className="speaker-transcript-header flex items-center justify-between gap-3 border-b border-indigo/10">
              <span className="text-xs font-semibold uppercase tracking-wide text-indigo-dark">
                {activeLabel}
              </span>
              {isSpeaking && (
                <motion.span
                  className="text-[0.65rem] font-medium text-indigo"
